...
def asymmetric_tanh_squash(
    z: torch.Tensor,
    low: torch.Tensor,
    high: torch.Tensor,
    default: torch.Tensor,
    slope_at_origin: float = 0.5,
    eps: float | None = None,
) -> torch.Tensor:
    """Squash the input to fall between the given bounds via two asymmetric piecewise hyperbolic
    tanget functions for values above and below zero.

    The mapping is defined as

    ```
    y = default + (high - default) * tanh(slope_at_origin / (high - default) * u),  if u >= 0
    y = default + (low - default) * tanh(slope_at_origin / (low - default) * u),   if u < 0
    ```
    It guarantees that zero input values are mapped to `default` output values, and has continuous
    first derivative.

    Args:
        z (Tensor): Input tensor to be squashed.
        low (Tensor): Lower bound.
        high (Tensor): Upper bound.
        default (Tensor): Default output value when input is zero, i.e., `z=0`.
        slope_at_origin (float): Slope of the squashing function at the origin. Defaults to `0.5`.
        eps (float or None): Small value for numerical stability. Defaults to `None`.

    Returns:
        Tensor: The squashed tensor.

    Notes:
        For default values exactly in the middle of the bounds, i.e., `default = (high + low) / 2`,
        this function is equivalent to a standard `tanh` squashing followed by scaling and shifting.
    """
    if eps is None:
        eps = torch.finfo(z.dtype).eps
    up = (high - default).clamp_min(eps)
    down = (low - default).clamp_max(-eps)
    return default + torch.where(
        z >= 0,
        up * torch.tanh((slope_at_origin / up) * z),
        down * torch.tanh((slope_at_origin / down) * z),
    )
