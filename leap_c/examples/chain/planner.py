...
class ChainPlanner(AcadosPlanner[AcadosDiffMpcCtx]):
    ...
    def __init__(
        ...
        Args:
            cfg: cfg: A configuration object containing high-level settings for the MPC problem,
                such as horizon length and maximum force. If not provided, a default config is used.
            params: An optional list of parameters to define the ocp object. If not provided,
                default parameters for the `Chain` system will be created based on the cfg.
            export_directory: Directory to export the acados ocp files.
        """
        self.cfg = ChainControllerConfig() if cfg is None else cfg
        if params is None:
            params = create_chain_params(
                self.cfg.param_interface, self.cfg.n_mass, self.cfg.N_horizon
            )
        ...
        diff_mpc = AcadosDiffMpcTorch(
            ocp, initializer, export_directory=export_directory, dtype=self.cfg.dtype
        )
