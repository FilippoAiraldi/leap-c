...
FexplType: TypeAlias = Callable[[struct_symSX, ca.SX, dict[str, np.ndarray | ca.SX], ca.SX], ca.SX]
...
def _define_nlp_solver(
    ...
    g = vertcat(xdot - f_expl_out, x["pos", -1] - p["p_last"], u)

    nlp = {"x": vertcat(x.cat, xdot, u), "f": 0, "g": g, "p": p.cat}
